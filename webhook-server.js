    ...
    req.on('end', () => {
      ...
      try {
        ...
        if (payload.ref === 'refs/heads/main') {
          console.log(`[${new Date().toISOString()}] Received push to main, deploying...`);
          deploy();
          res.writeHead(200);
          res.end('Deployment triggered');
        } else {
          console.log(`[${new Date().toISOString()}] Received push to ${payload.ref}, ignoring`);
          res.writeHead(200);
...
server.listen(PORT, () => {
  ...
  console.log(`[${new Date().toISOString()}] Secret configured: ${SECRET ? 'Yes' : 'No'}`);
});
