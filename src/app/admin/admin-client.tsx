...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "hero" | "about" | "experiences" | "projects" | "analytics"
  >(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("admin-active-tab");
      if (
        saved &&
        ["hero", "about", "experiences", "projects", "analytics"].includes(saved)
      ) {
        return saved as "hero" | "about" | "experiences" | "projects" | "analytics";
      }
    }
    return "hero";
  });
  ...
  const [conversionStatuses, setConversionStatuses] = useState<
    Record<string, { status: string; currentPage: number; totalPages: number; error?: string }>
  >({});
  const conversionPollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  const [thumbPan, setThumbPan] = useState({ x: 0, y: 0 });
  const [thumbDragging, setThumbDragging] = useState(false);
  const [thumbDragStart, setThumbDragStart] = useState({ x: 0, y: 0 });
  ...
  const uploadThumbnail = async () => {
    ...
    // Send crop info so backend can extract the visible 16:9 region
    const container = thumbPreviewRef.current;
    if (container) {
  ...
  /** Upload a file with byte-level progress tracking via XMLHttpRequest */
  const uploadFileWithProgress = useCallback(
    (file: File, fileIndex: number, totalFiles: number): Promise<{ ok: boolean; error?: string }> => {
      return new Promise((resolve) => {
        ...
        xhr.addEventListener("error", () => {
          resolve({ ok: false, error: `Failed to upload ${file.name}` });
        });

        xhr.open("POST", "/api/content/projects/upload");
        xhr.send(formData);
      });
    },
    []
  ...
  // Poll conversion statuses for all projects
  const pollConversionStatuses = useCallback(async () => {
    const pdfs = projectPdfs.length > 0 ? projectPdfs : [];
    if (pdfs.length === 0) return;

    let anyActive = false;
    for (const item of pdfs) {
      const safeName = item.title.toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
      try {
        const res = await fetch(`/api/content/projects/conversion-status?name=${encodeURIComponent(safeName)}`);
        if (!res.ok) continue;
        const data = await res.json();
        if (data.status !== "none") {
          setConversionStatuses((prev) => ({ ...prev, [safeName]: data }));
          if (data.status === "converting" || data.status === "queued") {
            anyActive = true;
          }
          // Refresh project list when conversion completes
          if (data.status === "done" && conversionStatuses[safeName]?.status === "converting") {
            loadProjectPdfs();
          }
        }
      } catch {}
    ...
    return anyActive;
  }, [projectPdfs, conversionStatuses, loadProjectPdfs]);
  ...
  // Start polling when projects tab is active
  useEffect(() => {
    if (activeTab !== "projects" || projectPdfs.length === 0) return;

    // Poll immediately
    pollConversionStatuses();
  ...
  }, [activeTab, projectPdfs.length, pollConversionStatuses]);
    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="container max-w-6xl py-8">
        ...
        <div className="mb-8 flex gap-2 overflow-x-auto border-b">
          {(["hero", "projects", "experiences", "about", "analytics"] as const).map(
            (tab) => (
              <button
            ...
            <div
              ...
              onDrop={async (e) => {
                ...
                for (let i = 0; i < pdfFiles.length; i++) {
                  ...
                  const result = await uploadFileWithProgress(file, i, pdfFiles.length);
                  if (!result.ok) {
                    setMessage({
                      type: "error",
                  ...
                  <div className="w-full max-w-xs space-y-1">
                    <div className="h-2 w-full overflow-hidden rounded-full bg-secondary">
                      <div
                        ...
                        style={{
                          width: `${
                            uploadProgress.bytesTotal && uploadProgress.bytesTotal > 0
                              ? Math.round((uploadProgress.bytesLoaded ?? 0) / uploadProgress.bytesTotal * 100)
                              : (uploadProgress.current / uploadProgress.total) * 100
                          }%`,
                        }}
                      />
                    </div>
                    {uploadProgress.bytesTotal != null && uploadProgress.bytesTotal > 0 && (
                      <p className="text-center text-muted-fg text-xs">
                        {Math.round((uploadProgress.bytesLoaded ?? 0) / uploadProgress.bytesTotal * 100)}%
                        {" — "}
                        {((uploadProgress.bytesLoaded ?? 0) / 1024 / 1024).toFixed(1)} / {(uploadProgress.bytesTotal / 1024 / 1024).toFixed(1)} MB
                      </p>
                    )}
                  </div>
                )}
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    for (let i = 0; i < files.length; i++) {
                      ...
                      const result = await uploadFileWithProgress(file, i, files.length);
                      if (!result.ok) {
                        setMessage({
                          type: "error",
                          text: result.error || "Upload failed",
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {(() => {
                          const safeName = item.title.toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
                          const status = conversionStatuses[safeName];
                          if (!status || status.status === "done" || status.status === "none") return null;
                          if (status.status === "error") {
                            return (
                              <div className="mt-2 flex items-center gap-2 text-xs text-red-500">
                                <Icons.X className="size-3.5" />
                                <span>Preview generation failed{status.error ? `: ${status.error}` : ""}</span>
                              </div>
                            );
                          }
                          if (status.status === "converting" || status.status === "queued") {
                            ...
                            return (
                              <div className="mt-2 space-y-1.5">
                                <div className="flex items-center gap-2 text-xs text-primary">
                                  ...
                                  <span>
                                    Generating previews {status.currentPage}/{status.totalPages} — {pct}%
                                  </span>
                                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-2xl rounded-lg border bg-bg p-6 shadow-lg">
            ...
            <div className="mb-4">
              ...
              {/* 16:9 preview matching homepage card */}
              <div
                ref={thumbPreviewRef}
                className="relative mx-auto aspect-[16/9] w-full overflow-hidden rounded-lg border bg-secondary/10"
                style={{ cursor: thumbDragging ? "grabbing" : "grab" }}
                onMouseDown={(e) => {
                  e.preventDefault();
                  setThumbDragging(true);
                  setThumbDragStart({ x: e.clientX - thumbPan.x, y: e.clientY - thumbPan.y });
                }}
                onMouseMove={(e) => {
                  if (!thumbDragging) return;
                  setThumbPan({ x: e.clientX - thumbDragStart.x, y: e.clientY - thumbDragStart.y });
                }}
                ...
                onTouchStart={(e) => {
                  const t = e.touches[0];
                  setThumbDragging(true);
                  setThumbDragStart({ x: t.clientX - thumbPan.x, y: t.clientY - thumbPan.y });
                }}
                onTouchMove={(e) => {
                  if (!thumbDragging) return;
                  const t = e.touches[0];
                  setThumbPan({ x: t.clientX - thumbDragStart.x, y: t.clientY - thumbDragStart.y });
                }}
                onTouchEnd={() => setThumbDragging(false)}
                onWheel={(e) => {
                  e.preventDefault();
                  setThumbZoom((z) => Math.max(1, Math.min(5, z + (e.deltaY > 0 ? -0.1 : 0.1))));
                }}
              >
                <img
                  alt="Thumbnail preview"
                  className="absolute h-full w-full origin-center select-none"
                  draggable={false}
                  src={thumbnailPreview.url}
                  style={{
                    objectFit: "cover",
                    transform: `scale(${thumbZoom}) translate(${thumbPan.x / thumbZoom}px, ${thumbPan.y / thumbZoom}px)`,
                  }}
                />
              </div>
              {/* Zoom slider */}
              <div className="mt-3 flex items-center gap-3">
                <span className="text-muted-fg text-xs">Zoom</span>
                <input
                ...
                />
                <span className="min-w-[3rem] text-right text-muted-fg text-xs">
                  {Math.round(thumbZoom * 100)}%
                </span>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <a
                ...
                href="https://analytics.fereshteh-hosseini.com"
                rel="noopener noreferrer"
                target="_blank"
              >
                Open full dashboard ↗
            ...
            {process.env.NEXT_PUBLIC_UMAMI_SHARE_URL ? (
              ...
              <div className="rounded-lg border p-8 text-center text-muted-fg">
                <p>Analytics not configured yet.</p>
                <p className="mt-2 text-sm">
                  Set NEXT_PUBLIC_UMAMI_SHARE_URL in your environment to embed
                  the dashboard here.
                </p>
              </div>
            )}
