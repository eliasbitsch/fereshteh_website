...
const DEFAULT_MAX_DIM = Math.max(1000, Number(process.env.PDF_MAX_DIM) || 16000);
const DEFAULT_MIN_WIDTH = Math.max(500, Number(process.env.PDF_MIN_WIDTH) || 1500);
const DEFAULT_QUALITY = Math.min(100, Math.max(1, Number(process.env.PDF_QUALITY) || 95));
const DEFAULT_EFFORT = Math.min(9, Math.max(0, Number(process.env.PDF_EFFORT) || 6));
const DEFAULT_LOSSLESS = process.env.PDF_LOSSLESS === '1' || process.env.PDF_LOSSLESS === 'true';
...
async function convertPDFsToAVIF() {
  ...
  if (!existsSync(projectsDir)) {
    console.log(`Source directory ${projectsDir} does not exist. Nothing to convert.`);
    return;
  }
  ...
  for (const pdfFile of pdfFiles) {
    ...
    try {
      ...
      console.log(`  Rendered dimensions: ${metadata.width}x${metadata.height}`);
      ...
      if (metadata.width && metadata.height) {
        ...
        if (resultingWidth < minWidth) {
          ...
          console.log(`  Enforcing minimum width of ${minWidth}px for text sharpness`);
        }

      }

      // High quality JPEG: no height limit, good compression
      const pipeline = image.resize({ ...resizeOptions, fit: "inside", withoutEnlargement: true });
      await pipeline.jpeg({ quality: DEFAULT_QUALITY, mozjpeg: true }).toFile(outputPath);
